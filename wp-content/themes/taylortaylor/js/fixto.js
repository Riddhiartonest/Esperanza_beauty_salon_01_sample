    ...
    * Copyright (c) 2012 Burak Barakaci; Licensed MIT */
    ...
    $.extend(FixToContainer.prototype, {
        ...
        _onscroll: function _onscroll() {
            ...
            //ale_mod
            if(this.options.fixToBottom !== false && this.child.offsetHeight > window.innerHeight){
                
                if(this.child.offsetHeight > this.parent.offsetHeight) return;
                
                offset_bottom = 20;
                
                if(this.fixed && this._scrollTop < this._fullOffset('offsetTop', this._ghostNode) - base_pos){
                    this._unfix();
                    this._adjust();
                }else{
                    if(!this.fixed && this._scrollTop > this.child.offsetHeight + this._fullOffset('offsetTop', this.parent) - window.innerHeight + offset_bottom){ 
                        this._fix();
                        this._adjust();
                    }else{
                        this._adjust();
        ...
        _adjust: function _adjust() {
            ...
            if(this.options.fixToBottom !== false && this.child.offsetHeight > window.innerHeight){
                ...
                base_pos =  window.innerHeight - this.child.offsetHeight - offset_bottom;
                diff = (this._parentBottom - this._scrollTop) - (this.child.offsetHeight + computedStyle.toFloat(childStyles.marginBottom) + mindTop + this.options.top) + (window.innerHeight - this.child.offsetHeight ) - base_pos - offset_bottom;
                
                if(!this.fixed){
                    diff = 0;
                }else if(diff > base_pos){
                    diff = base_pos;
                }else if(diff > 0){
                    diff = 0;
                }
